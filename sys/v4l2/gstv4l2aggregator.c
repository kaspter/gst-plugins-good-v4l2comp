...
gst_v4l2_aggregator_check_pads_ready (GstV4l2Aggregator * self)
{
  ...
  for (l = sinkpads; l != NULL; l = l->next) {
    ...
    if (self->priv->peer_latency_live &&
        !gst_v4l2_aggregator_pad_queue_is_empty (pad))
      self->priv->first_buffer = FALSE;

    if (gst_v4l2_aggregator_pad_queue_is_empty (pad) && !pad->priv->eos) {
      PAD_UNLOCK (pad);
      goto pad_not_ready;
    }
    PAD_UNLOCK (pad);
...
gst_v4l2_aggregator_do_seek (GstV4l2Aggregator * self, GstEvent * event)
{
  ...
  gst_v4l2_aggregator_forward_event_to_all_sinkpads (self, event, flush, FALSE, &evdata);
  event = NULL;
